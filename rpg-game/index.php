...
switch ($action) {
    ...
    case 'create_hero':
        include 'views/hero_form.php'; // Kita akan buat file ini di langkah 2
        break;
    ...
    case 'store_hero':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Tentukan Base Stats Awal berdasarkan Job
            // Logic sederhana: Warrior kuat STR, Mage kuat INT
            $str = 10; $agi = 10; $int = 10;

            if ($job == 'Warrior') { 
                $str = 25; $agi = 15; $int = 10; 
            } elseif ($job == 'Mage') { 
                $str = 10; $agi = 15; $int = 25; 
            } elseif ($job == 'Assassin') { 
                $str = 15; $agi = 25; $int = 10; 
            }

            // Panggil Repository untuk simpan
            $heroRepo->createHero($name, $job, $str, $agi, $int);
            
            // Balik ke halaman pilih hero
            header("Location: index.php?action=home");
        }
        break;
    case 'edit_hero':
        // Ambil data hero berdasarkan ID untuk ditampilkan di form
        $hero = $heroRepo->getHeroById($_GET['id']);
        if ($hero) {
            include 'views/hero_edit.php';
        } else {
    ...
    case 'update_hero':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // LOGIC: Reset Base Stats jika Job berubah
            // (Kita samakan logic-nya dengan create_hero)
            $str = 10; $agi = 10; $int = 10;
            ...
            // Panggil Repo untuk update data & stats
            $heroRepo->updateHero($id, $name, $job, $str, $agi, $int);
    ...
    case 'delete_hero':
        $id = $_GET['id'];
        $heroRepo->deleteHero($id);
        
        // Hapus session kalau hero yang dihapus lagi dimainkan
        if (isset($_SESSION['hero_id']) && $_SESSION['hero_id'] == $id) {
    ...
    case 'store':
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($name)) {
                die("Error: Nama item tidak boleh kosong!");
            }
            if ($price < 0) {
                die("Error: Harga tidak boleh minus!");
            }
