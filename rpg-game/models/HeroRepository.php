...
class HeroRepository {
    ...
    public function adventure($heroId) {
        ...
        $reqXp = 100 + ($currentLevel - 1) * 150;

        while ($currentXp >= $reqXp) {
            $currentXp -= $reqXp; // Reset XP (Sisa XP dibawa ke level baru)
            $currentLevel++;      // Naik Level
            
            // BONUS LEVEL UP: Tambah Base Stats!
            $hero->base_str += 5;
            $hero->base_agi += 2;
            $hero->base_int += 2;
            
            // Update Base Max HP di Database juga (biar sinkron)
            $hero->max_hp = $hero->base_str * 20;
            $hero->max_mana = $hero->base_int * 15;

            // Full Heal sebagai hadiah Level Up
            $hero->current_hp = $hero->max_hp;
            $hero->current_mana = $hero->max_mana;
            $hpLoss = 0; // Gak jadi luka
            
            // Update Req XP untuk loop berikutnya
            $reqXp = 100 + ($currentLevel - 1) * 150;
            
            $levelUpMsg .= " 🎉 LEVEL UP ke $currentLevel! All Stats Up!";
        }
