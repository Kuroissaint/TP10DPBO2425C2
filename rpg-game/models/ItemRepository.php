...
class ItemRepository {
    ...
    public function getItemById($id) {
        // Query sama kayak getAllItems, tapi pake WHERE id = ?
        $query = "
            SELECT 
                i.*, 
                w.attack_power, w.element,
        ...
        // Factory Logic
        switch ($row['type']) {
            case 'Weapon': return new Weapon($row);
            case 'Accessory': return new Accessory($row);
            case 'Consumable': return new Consumable($row);
            default: return null;
        }
    }
    ...
    // --- UPDATE WEAPON ---
    public function updateWeapon($id, $name, $price, $atk, $element) {
        try {
            $this->conn->beginTransaction();
            
            // 1. Update Bapaknya (Items)
            $stmt1 = $this->conn->prepare("UPDATE items SET name=?, price=? WHERE id=?");
            $stmt1->execute([$name, $price, $id]);

            // 2. Update Anaknya (Weapons)
            $stmt2 = $this->conn->prepare("UPDATE weapons SET attack_power=?, element=? WHERE item_id=?");
            $stmt2->execute([$atk, $element, $id]);
    ...
    // --- UPDATE ACCESSORY ---
    public function updateAccessory($id, $name, $price, $str, $agi, $int) {
        try {
            $this->conn->beginTransaction();
            $stmt1 = $this->conn->prepare("UPDATE items SET name=?, price=? WHERE id=?");
            $stmt1->execute([$name, $price, $id]);

            $stmt2 = $this->conn->prepare("UPDATE accessories SET bonus_str=?, bonus_agi=?, bonus_int=? WHERE item_id=?");
            $stmt2->execute([$str, $agi, $int, $id]);
    ...
    // --- UPDATE CONSUMABLE ---
    public function updateConsumable($id, $name, $price, $hp, $mana) {
        try {
            $this->conn->beginTransaction();
            $stmt1 = $this->conn->prepare("UPDATE items SET name=?, price=? WHERE id=?");
            $stmt1->execute([$name, $price, $id]);

            $stmt2 = $this->conn->prepare("UPDATE consumables SET recover_hp=?, recover_mana=? WHERE item_id=?");
            $stmt2->execute([$hp, $mana, $id]);
